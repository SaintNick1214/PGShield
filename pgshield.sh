...
question1() {
  touch /opt/var/plexguide/auth.bypass

  a7=$(cat /opt/var/plexguide/auth.bypass)
  if [[ "$a7" != "good" ]]; then shieldcheck; fi
  echo good >/opt/var/plexguide/auth.bypass

  touch /opt/var/plexguide/pgshield.emails
  mkdir -p /opt/var/plexguide/auth/

  domain=$(cat /opt/var/plexguide/server.domain)
...
phase1() {
  ...
  4)
    ...
    touch /opt/var/plexguide/pgshield.emails
    efg=$(cat "/opt/var/plexguide/pgshield.emails")
    if [[ "$efg" == "" ]]; then
      echo
    ...
    file="/opt/var/plexguide/auth.idset"
    if [ ! -e "$file" ]; then
      echo
    ...
    touch /opt/var/plexguide/server.ports
    ports=$(cat "/opt/var/plexguide/server.ports")
    ...
    touch /opt/var/plexguide/pgshield.compiled
    rm -r /opt/var/plexguide/pgshield.compiled
    while read p; do
      echo -n "$p," >>/opt/var/plexguide/pgshield.compiled
    done </opt/var/plexguide/pgshield.emails

    ansible-playbook /opt/pgshield/pgshield.yml

    pgstatus=$(docker ps | grep "\<thomseddon\>" | awk '{ print $2}')
    if [[ "$pgstatus" != "" ]]; then touch /opt/var/plexguide/oauth.exists
    else rm -rf /opt/var/plexguide/oauth.exists; fi
...
appexempt() {
  ...
  ls -l /opt/var/plexguide/auth | awk '{ print $9 }' >/opt/var/plexguide/pgshield.ex15
...
phase3() {
  ...
  2)
    ...
    ;;
  3)
    emptycheck=$(cat /opt/var/plexguide/pgshield.ex15)
    if [[ "$emptycheck" == "" ]]; then
      echo
      read -p 'No Apps have PGShield Disabled! Exiting | Press [ENTER]'
      appexempt
    fi
    rm -rf /opt/var/plexguide/auth/*
    echo ""
...
phase31() {
  touch /opt/var/plexguide/app.list
  while read p; do
    sed -i -e "/$p/d" /opt/var/plexguide/app.list
  done </opt/var/plexguide/pgshield.ex15

  ### Blank Out Temp List
  rm -rf /opt/var/plexguide/program.temp && touch /opt/var/plexguide/program.temp

  ### List Out Apps In Readable Order (One's Not Installed)
  num=0
  while read p; do
    echo -n $p >>/opt/var/plexguide/program.temp
    echo -n " " >>/opt/var/plexguide/program.temp
    num=$((num + 1))
    if [ "$num" == 7 ]; then
      num=0
      echo " " >>/opt/var/plexguide/program.temp
    fi
  done </opt/var/plexguide/app.list

  notrun=$(cat /opt/var/plexguide/program.temp)
...
EOF
  ...
  grep -w "$typed" /opt/var/plexguide/program.temp >/opt/var/plexguide/check55.sh
  usercheck=$(cat /opt/var/plexguide/check55.sh)
  ...
  touch /opt/var/plexguide/auth/$typed
  echo
...
EOF
  ...
  grep -w "$typed" /opt/var/plexguide/pgshield.ex15 >/opt/var/plexguide/check55.sh
  usercheck=$(cat /opt/var/plexguide/check55.sh)
  ...
  rm -rf /opt/var/plexguide/auth/$typed
  echo
...
phase2() {
  ...
  1)
    ...
    usercheck=$(cat /opt/var/plexguide/pgshield.emails | grep $typed)
    if [[ "$usercheck" != "" ]]; then
      read -p 'User Already Exists! | Press [ENTER] ' note </dev/tty
      email
    fi
    read -p 'User Added | Press [ENTER] ' note </dev/tty
    echo "$typed" >>/opt/var/plexguide/pgshield.emails
    email
    ;;
  2)
    echo
    read -p 'User Email to Remove | Press [ENTER]: ' typed </dev/tty
    testremove=$(cat /opt/var/plexguide/pgshield.emails | grep $typed)
    if [[ "$testremove" == "" ]]; then
      read -p 'User does not exist | Press [ENTER] ' typed </dev/tty
      email
    fi
    sed -i -e "/$typed/d" /opt/var/plexguide/pgshield.emails
    echo ""
    echo "NOTE: Does not take effect until PG Shield is redeployed!"
  ...
  3)
    ...
    echo ""
    cat /opt/var/plexguide/pgshield.emails
    echo
    ...
    ;;
  4)
    test=$(cat /opt/var/plexguide/pgshield.emails | grep "@")
    if [[ "$test" == "" ]]; then email; fi
    docker stop oauth
    rm -r /opt/var/plexguide/pgshield.emails
    touch /opt/var/plexguide/pgshield.emails
    echo
    docker stop oauth
...
shieldcheck() {
  domaincheck=$(cat /opt/var/plexguide/server.domain)
  touch /opt/var/plexguide/server.domain
  touch /tmp/portainer.check
  rm -r /tmp/portainer.check
  cname="portainer"
  if [[ -f "/opt/var/plexguide/portainer.cname" ]]; then
    cname=$(cat "/opt/var/plexguide/portainer.cname")
  fi
